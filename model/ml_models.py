...
class ML_models:
    ...
    def ml_train(self):
        ...
        for i in range(self.k_fold):
            ...
            if self.concat:
                # dyn = pd.read_csv(
                #     "./data/csv/" + str(train_hids[0]) + "/dynamic.csv", header=[0, 1]
                # )
                try:
                    dyn = pd.read_csv(
                        "./data/csv/" + str(train_hids[0]) + "/dynamic.csv",
                        header=[0, 1],
                    )
                except pd.errors.EmptyDataError:
                    dyn = pd.DataFrame({"CHART": [], "MEDS": [], "LAB": [], "COND": []})
    ...
    def train_model(self, X_train, Y_train, X_test, Y_test):
        ...
        if self.model_type == "Gradient Bossting":
            ...
            X_train = pd.get_dummies(
                X_train,
                prefix=["gender", "ethnicity", "insurance"],
                columns=["gender", "ethnicity", "insurance"],
            )
            ...
            self.save_outputImp(
                Y_test, prob[:, 1], logits[:, 1], model.coef_[0], X_train.columns
            )
            ...
            self.loss(prob[:, 1], np.asarray(Y_test), logits, False, True)
            self.save_outputImp(
                Y_test, prob[:, 1], logits, model.feature_importances_, X_train.columns
            )
        ...
        for sample in ids:
            ...
            if X_df.empty:
                ...
                X_df = pd.concat(
                    [
                        X_df,
                        pd.concat([pd.concat([dyn_df, stat], axis=1), demo], axis=1),
                    ],
                    axis=0,
                )
            if y_df.empty:
