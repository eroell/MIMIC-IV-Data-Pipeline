...
class Generator:
    ...
    def generate_adm(self):
        data = pd.read_csv(
            f"./data/cohort/{self.cohort_output}.csv.gz",
            compression="gzip",
            header=0,
            index_col=None,
        )
    ...
    def mortality_length(self, include_time, predW):
        ...
        if self.feat_med:
            ...
            self.meds.loc[self.meds.stop_time > include_time, "stop_time"] = (
                include_time
            )
    ...
    def readmission_length(self, include_time):
        ...
        if self.feat_chart:
            ...
            self.chart["start_time"] = (
                self.chart["start_time"] - self.chart["select_time"]
            )
    ...
    def smooth_meds(self, bucket):
        ...
        for i in tqdm(range(0, self.los, bucket)):
            ...
            if self.feat_out:
                sub_out = (
                    self.out[
                        (self.out["start_time"] >= i)
                        & (self.out["start_time"] < i + bucket)
                    ]
                    .groupby(["stay_id", "itemid"])
                    .agg({"subject_id": "max"})
                )
        ...
        for hid in tqdm(self.hids):
            ...
            if self.feat_med:
                ...
                if df2.shape[0] == 0:
                    ...
                    amount.columns = pd.MultiIndex.from_product(
                        [["MEDS"], amount.columns]
                    )
                else:
                    ...
                    add_indices = pd.Index(range(los)).difference(df2.index)
                    add_df = pd.DataFrame(
                        index=add_indices, columns=df2.columns
                    ).fillna(np.nan)
                    df2 = pd.concat([df2, add_df])
                    df2 = df2.sort_index()
                    df2 = df2.ffill()
                    ...
                    # print(df2.head())
                    dataDic[hid]["Med"]["signal"] = df2.iloc[:, 0:].to_dict(
                        orient="list"
                    )
                    dataDic[hid]["Med"]["rate"] = rate.iloc[:, 0:].to_dict(
                        orient="list"
                    )
                    ...
                    feat_df = pd.DataFrame(
                        columns=list(set(feat) - set(amount.columns))
                    )
                    #                 print(feat)
            ...
            if self.feat_chart:
                ...
                if df2.shape[0] == 0:
                    ...
                    df2[df2 < 0] = 0
                    # print(df2.head())
                    dataDic[hid]["Chart"]["signal"] = df2.iloc[:, 0:].to_dict(
                        orient="list"
                    )
                    dataDic[hid]["Chart"]["val"] = val.iloc[:, 0:].to_dict(
                        orient="list"
                    )
