...
def callibrate(inputFile, outputFile):
    ...
    optimizer = optim.LBFGS(
        [temperature], lr=0.0001, max_iter=100000, line_search_fn="strong_wolfe"
    )
    ...
    def _eval():
        ...
        loss = criterion(
            T_scaling(
                torch.tensor(output_dict["Logits"]).type(torch.FloatTensor).to(device),
                temperature,
            ),
            torch.tensor(output_dict["Labels"]).type(torch.FloatTensor).to(device),
        )
        # print(loss)
        loss.backward()
    ...
    loss = evaluation.Loss(
        device, True, True, True, True, True, True, True, True, True, True, True
    )
    print("BEFORE CALLIBRATION")
    if "Prob" in output_dict.columns:
        out_loss = loss(
            torch.tensor(output_dict["Prob"]),
            torch.tensor(output_dict["Labels"]),
            torch.tensor(output_dict["Logits"]),
            False,
        )
        output_dict["Prob"] = prob.data.cpu().numpy()
    else:
        out_loss = loss(
            sm(torch.tensor(output_dict["Logits"]).to(device)),
            torch.tensor(output_dict["Labels"]),
            torch.tensor(output_dict["Logits"]),
            False,
        )
